...
import { ExportHistoryPanel } from './ExportHistoryPanel.tsx';
...
import { 
  ...
  safeFetchBase64Image 
} from '../utils/codeGenerator.ts';
import { 
  ReceiptDocument, 
...
export default function DocumentStudio({ 
  ...
  const [exportModalRecordData, setExportModalRecordData] = useState<any>(null);
  ...
  const [aoaHeadOffice, setAoaHeadOffice] = useState('Yaoundé Mbankolo, Cameroon');
  ...
  const openExportModalForActiveSubMode = () => {
    ...
    switch (contractSubMode) {
      case 'works':
        moduleType = 'civil_works';
        recordId = contractNo || 'MADECC-2026-LA-089';
        title = `Civil Works Labour Agreement - ${contractProject}`;
        recordData = {
          recordId,
          title,
          ...
          totalAmount: parseFloat(contractValue) || 2200000,
        ...
        break;
      case 'articles':
        moduleType = 'articles_of_association';
        recordId = `AOA-2026-${aoaCompanyName.replace(/\s+/g, '_')}`;
        title = `Articles of Association - ${aoaCompanyName}`;
        recordData = {
          recordId,
          companyName: aoaCompanyName,
          registeredOffice: aoaHeadOffice,
          shareCapital: aoaShareCapital,
          version: '1.0',
          adoptionDate: new Date().toLocaleDateString('en-GB'),
          shareholders: [
            { name: aoaInitialManager, shares: parseInt(aoaSharesCount) || 1000, percentage: 100 },
          ],
          articles: (aoaGeneratedData?.articles || getOfflineFallbackArticles().articles).map((a: any) => ({
            articleNumber: a.number || 1,
            title: a.title,
            clauses: [{ clauseNumber: '1.1', content: a.content }],
          })),
        };
        break;
      case 'blueprints':
        moduleType = 'blueprints';
        recordId = blueprintCode || 'BLU-2026-F4BA-012';
        title = blueprintTitle;
        recordData = {
          recordId,
          drawingCode: blueprintCode,
          revision: 'v1.0',
          title: blueprintTitle,
          projectName: contractProject || 'MADECC Construction',
          discipline: 'Structural Engineering',
          scale: blueprintScale,
          materialsSpecs: blueprintMaterialsSpecs,
          structuralNotes: blueprintNotes,
          date: new Date().toLocaleDateString('en-GB'),
          leadEngineer: blueprintDesignedBy,
        };
        break;
      case 'safety':
        moduleType = 'safety_inspections';
        recordId = safetyReportCode || 'HSE-2026-089';
        title = `HSE Safety Audit - ${safetySiteLocation}`;
        recordData = {
          recordId,
          inspectionCode: safetyReportCode,
          projectName: 'Civil Construction Site',
          siteLocation: safetySiteLocation,
          inspectionDate: safetyDate,
          inspectorName: safetyInspector,
          contractorName: 'MADECC Subcontractors',
          status: safetyStatus === 'Pass' ? 'Passed Compliance' : 'Requires Correction',
          ppeCompliancePercentage: parseFloat(safetyPpeCompliance) || 92,
          hazardsIdentified: safetyHazardsIdentified,
          mandatedActions: safetyCorrectiveActions,
          items: [
            { itemNo: 1, checkItem: 'PPE Safety Compliance', category: 'PPE', status: 'Pass', observation: 'High compliance observed', correctiveAction: 'Maintain current standard' },
            { itemNo: 2, checkItem: 'Site Hazards & Trench Shoring', category: 'Civil Safety', status: safetyStatus === 'Pass' ? 'Pass' : 'Requires Attention', observation: safetyHazardsIdentified, correctiveAction: safetyCorrectiveActions },
          ],
        };
        break;
      case 'pedagogical':
        moduleType = 'pedagogical_lessons';
        recordId = `PL-2026-${pedagogicalTopic.slice(0, 10).replace(/\s+/g, '_')}`;
        title = `Pedagogical Lesson Plan - ${pedagogicalTopic}`;
        recordData = {
          recordId,
          subject: 'Civil Engineering',
          topic: pedagogicalTopic,
          classLevel: pedagogicalLevel,
          duration: pedagogicalDuration,
          syllabusUnit: pedagogicalSyllabusUnit,
          cbaGoal: pedagogicalCbaGoal,
          schoolName: 'Technical Institute',
          teacherName: representativeName,
          teacherCoachingAdvice: pedagogicalTeacherCoaching,
          lessonNumber: 1,
          term: 'Term 1',
    ...
    setExportModalModuleType(moduleType);
    setExportModalRecordId(recordId);
    setExportModalDocumentTitle(title);
    setExportModalRecordData(recordData);
    setIsExportPreviewModalOpen(true);
  };
  ...
  // ==========================================
  // --- SIGNING STAMPS & CODE GENERATION ---
  // ==========================================
  const fetchBase64Image = async (url: string): Promise<string> => {
    return safeFetchBase64Image(url);
  };
  ...
  const handleLoadReceiptToPreview = async (receipt: any) => {
    ...
    // Generate local offline barcode
    if (receipt.receiptNo) {
      const barcodeBase64 = generateBarcodeBase64(receipt.receiptNo);
      if (barcodeBase64) {
        canonicalDoc.barcodeBase64 = barcodeBase64;
      }
    }
  ...
  const handleEditContract = (contract: any) => {
    ...
    // Regenerate QR base64 locally
    if (contract.verificationToken) {
  ...
  const applyContractSignature = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Generate local offline QR code
      const verificationUrl = `${window.location.origin}/?verify=${token}`;
      const base64Qr = await generateQrCodeBase64(verificationUrl);
  ...
  const applyReceiptSignature = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate local offline QR code and barcode
      const verificationUrl = `${window.location.origin}/?verify=${token}`;
      const base64Qr = await generateQrCodeBase64(verificationUrl);
      const base64Barcode = generateBarcodeBase64(savedDoc.receiptNo);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500" id="legal-document-studio-root">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">
            {mode === 'contracts' ? 'Enterprise Document & Export Studio' : 'Digital Receipts Studio'}
          </h1>
          <p className="text-xs text-slate-400">
            {mode === 'contracts' 
              ? 'Generate fully compliant A4 PDF and Word (.DOCX) exports across Civil Works, Statutes, Blueprints, Safety, and Pedagogical plans.' 
        ...
        <div className="bg-slate-950 p-1 rounded-xl border border-slate-800 flex items-center gap-1">
          <button
            type="button"
            onClick={() => setActiveStudioSubTab('studio')}
            className={`text-xs px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 cursor-pointer transition-all ${
              activeStudioSubTab === 'studio' ? 'bg-amber-500 text-slate-950 shadow shadow-amber-500/10' : 'text-slate-400 hover:text-white'
            }`}
          >
            <Scale className="w-3.5 h-3.5" /> Document Studio
          </button>
          <button
            type="button"
            onClick={() => setActiveStudioSubTab('audit_history')}
            className={`text-xs px-3 py-1.5 rounded-lg font-bold flex items-center gap-1.5 cursor-pointer transition-all ${
              activeStudioSubTab === 'audit_history' ? 'bg-amber-500 text-slate-950 shadow shadow-amber-500/10' : 'text-slate-400 hover:text-white'
            }`}
          >
            <Clock className="w-3.5 h-3.5" /> Export Audit Trail
          </button>
        </div>
      </div>

      {activeStudioSubTab === 'audit_history' ? (
        <ExportHistoryPanel showToast={showToast} />
      ) : (
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 space-y-5 shadow-lg">
              ...
              {contractSubMode === 'works' ? (
                ...
                <div className="space-y-4">
                    ...
                    >
                      ...
                      {projects.map((p) => (
                        <option key={p.id} value={p.title}>
                          {p.title} (Budget: {parseFloat(p.budget || '0').toLocaleString()} {(p as any).currency || 'XAF'})
                        </option>
                      ))}
                  ...
                  <div className="border-t border-slate-900/60 pt-4 space-y-4">
                    <span className="text-[11px] text-amber-500 font-bold uppercase tracking-wider block">
                      Agreed Balance & Payment Milestones ({contractCurrency})
                    </span>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. Yaoundé Mbankolo, Cameroon"
                        className="w-full bg-slate-900 border border-slate-800 rounded-xl py-2 px-3 text-xs text-white outline-none focus:border-amber-500"
                        value={aoaHeadOffice}
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-wrap gap-4 items-center justify-between shadow-lg">
            <span className="text-xs font-mono font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
              <Printer className="w-4 h-4 text-amber-500 animate-pulse" /> Centralized Export Hub
            </span>
            ...
            <div className="flex flex-wrap gap-2.5">
              <button
                ...
                className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-black py-2 px-3.5 rounded-xl text-xs uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-md shadow-amber-500/10 transition-colors"
              >
                <Download className="w-3.5 h-3.5 text-slate-950" /> Export A4 PDF
              </button>
              <button
                type="button"
                onClick={openExportModalForActiveSubMode}
                className="bg-blue-600 hover:bg-blue-700 text-white font-black py-2 px-3.5 rounded-xl text-xs uppercase tracking-wider flex items-center gap-1.5 cursor-pointer shadow-md shadow-blue-600/10 transition-colors"
              >
                <FileDown className="w-3.5 h-3.5 text-white" /> Export Word (.DOCX)
              </button>
              <button
                type="button"
              ...
              <button
                type="button"
                onClick={() => setActiveStudioSubTab('audit_history')}
                className="bg-slate-900 hover:bg-slate-800 text-slate-300 font-extrabold py-2 px-3.5 rounded-xl text-xs uppercase tracking-wider flex items-center gap-1.5 cursor-pointer border border-slate-800 transition-colors"
              >
                <Clock className="w-3.5 h-3.5 text-amber-400" /> Export History
              </button>
            </div>
          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-start pb-4 border-b border-slate-200">
                  <div className="space-y-1">
                    ...
                    <p className="font-sans text-[8px] text-slate-400">Yaoundé Mbankolo, Cameroon</p>
                  </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-900 text-slate-400 font-mono uppercase text-[10px] tracking-wider border-b border-slate-900">
                    ...
                    <th className="py-3.5 px-4 font-bold text-right">Valuation ({contractCurrency})</th>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-900 text-slate-400 font-mono uppercase text-[10px] tracking-wider border-b border-slate-900">
                    ...
                    <th className="py-3.5 px-4 font-bold text-right">Invoice Total ({contractCurrency})</th>
                    <th className="py-3.5 px-4 font-bold text-right">Paid Amount ({contractCurrency})</th>
            ...
            contractSubMode === 'works' ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 text-[9px] text-slate-700 leading-normal">
                  ...
                  <p><strong>ARTICLE 3: CAPITAL & SHARES.</strong> The share capital is fixed at <strong>{aoaShareCapital || `10,000,000 ${contractCurrency}`}</strong>, divided into <strong>{aoaSharesCount || '1,000'}</strong> shares of nominal value <strong>{aoaShareValue || `10,000 ${contractCurrency}`}</strong> each.</p>
      ...
      {/* Centralized Export Preview Modal */}
      <ExportPreviewModal
        isOpen={isExportPreviewModalOpen}
        onClose={() => setIsExportPreviewModalOpen(false)}
        moduleType={exportModalModuleType}
        recordId={exportModalRecordId}
        documentTitle={exportModalDocumentTitle}
